...
getPeriodOrdersReport: (params = {}) => {
  const query = new URLSearchParams(params).toString();
  return request(`/api/admin/reports/period-orders${query ? `?${query}` : ''}`);
},
getStatusOrdersReport: (params = {}) => {
  const query = new URLSearchParams(params).toString();
  return request(`/api/admin/reports/status-orders${query ? `?${query}` : ''}`);
},
