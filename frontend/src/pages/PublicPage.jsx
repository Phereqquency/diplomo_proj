...
export default function PublicPage() {
  ...
  const handleSubmit = async () => {
    setError('');
    try {
      const zayavka = await api.createZayavka({ uslugi_ids: selectedUslugi, tz: tz });
      setSuccess(true);
  ...
  if (success) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-10 w-full max-w-md text-center">
        ...
        <p className="text-gray-400 text-sm mb-6">Мы свяжемся с вами в ближайшее время.</p>
        <div className="flex flex-col gap-3">
          <button
            onClick={() => { setSuccess(false); setStep(1); setSelectedUslugi([]); setForm({ imya: '', email: '', messenger: '' }); setTz(''); }}
          ...
          <a href="/orders" className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-xl text-sm font-medium transition-all">
            Мои заявки
          </a>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-10 w-full max-w-md text-center">
          ...
          <div className="flex flex-col gap-3">
            <a href="/login" className="block w-full bg-gray-900 hover:bg-gray-800 text-white py-3 rounded-xl font-medium transition-all">
              Войти
            </a>
            <a href="/register" className="block w-full bg-gray-100 text-gray-700 py-3 rounded-xl font-medium hover:bg-gray-200 transition-all">
              Зарегистрироваться
            </a>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200">
        ...
        {admin ? (
          <a href="/orders" className="text-sm text-gray-500 hover:text-gray-900 font-medium transition-colors">
            Мои заявки →
          </a>
        ) : (
      ...
      <div className="max-w-2xl mx-auto px-5 py-8">
        ...
        <div className="flex items-center gap-2 mb-6">
          {[
            { n: 1, label: 'Выбор услуг' },
            { n: 2, label: 'Ваши данные' },
          ].map(({ n, label }, i) => (
          ...
          {step === 2 && (
            <>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={handleSubmit} disabled={!form.imya || !form.email}
